...
import { runEvaluation } from '@/app/p/[projectId]/no-meeting/actions';
...
import type { NoMeetingData } from '@/lib/no-meeting/queries';
import type { AgendaItem, MeetingRequest } from '@/lib/no-meeting/types';
import { SectionHead, VerdictStamp, outlineBtn } from './atoms';
...
export function NoMeetingToday({
  ...
  const teamsync = connections.teamsync;
  ...
  // 절약 "시간" 이 아니라 열리지 않은 회의 수를 센다.
  // 예정 인원 × 예정 시간은 신청자의 자기신고값이라 곱해서 자랑할 값이 아니다.
  const judged = evaluations.filter((e) => e.outcome !== null).length;
  ...
  return (
    <div>
        ...
        <div className="mx-auto max-w-[1080px] px-5 py-10 sm:px-10 sm:py-12">
          ...
          <div className="mt-8 grid gap-px overflow-hidden rounded-sm border border-[var(--navy-border-2)] bg-[var(--navy-border-2)] sm:grid-cols-3">
            ...
            <HeroStat
              ...
              unit="건"
              note={judged > 0 ? `판정 ${judged}건 중` : '아직 판정한 회의가 없습니다'}
            />
            <HeroStat label="판정 대기" value={requests.length} unit="건" note="아직 판정하지 않은 요청" />
          </div>

          {pendingDecisions > 0 && (
            <Link
              href={`/p/${projectId}/inbox`}
              className="mt-5 inline-flex h-11 items-center rounded-sm bg-white px-5 text-[15px] font-bold text-[var(--navy)] hover:no-underline"
            >
              결정 인박스 열기 · {pendingDecisions}건 →
            </Link>
          )}
        </div>
      </section>
      ...
      <div className="mx-auto w-full max-w-[1080px] px-5 py-10 sm:px-10">
        ...
        {/* ── 판정 대기 큐 ──────────────────────────────────────── */}
        <div className="mt-12">
          <SectionHead count={`${requests.length}건`}>판정 대기</SectionHead>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-dashed border-[var(--rule)] pb-3">
          <span className="flex items-center gap-2 text-[14.5px] text-[var(--ink-soft)]">
            <span
              className={`block h-2 w-2 shrink-0 rounded-full ${teamsync.status === 'CONNECTED' ? 'live-dot' : ''}`}
        ...
        {requests.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-14 text-center">
            <span className="stamp px-4 py-1.5 text-[13px] font-bold">queue empty</span>
          ...
          <ul className="mt-6 space-y-4">
            {requests.map((r) => (
              <RequestCard key={r.id} req={r} projectId={projectId} evidenceCount={(data.evidenceByRequest[r.id] ?? []).length} />
            ))}
          </ul>
        )}
...
const AGENDA_KIND: Record<AgendaItem['kind'], string> = {
  INFO: '확인', QUESTION: '질의', DECISION: '결정',
};

// ── 회의 요청서 한 장 ─────────────────────────────────────────────

function RequestCard({
  req, projectId, evidenceCount,
}: { req: MeetingRequest; projectId: string; evidenceCount: number }) {
  const run = runEvaluation.bind(null, projectId, req.id);
  const top = [...req.typeCandidates].sort((a, b) => b.score - a.score)[0];
    ...
    <li className="rounded-sm border border-[var(--rule)] bg-white shadow-[0_1px_3px_rgba(20,22,26,.06)]">
      <div className="flex flex-col gap-5 px-5 py-5 sm:flex-row sm:items-start sm:px-7 sm:py-6">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-[21px] font-bold leading-snug sm:text-[23px]">{req.title}</h3>
          {req.purposeText && (
            <p className="mt-1.5 whitespace-pre-line text-[15.5px] leading-relaxed text-[var(--ink-soft)]">
              {req.purposeText}
            </p>
          )}
          ...
          <div className="tabular mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[13.5px] text-[var(--ink-faint)]">
            ...
            <span>{req.attendeeCandidates.length}명</span>
            <span>{req.plannedMinutes}분</span>
            <span>근거 {evidenceCount}건</span>
            {top && <span>{TYPE[top.type].code} 후보 {top.score.toFixed(2)}</span>}
          </div>

          {req.agenda.length > 0 && (
            <ul className="mt-4 space-y-1.5 border-t border-dashed border-[var(--rule)] pt-3">
              {req.agenda.map((a) => (
                <li key={a.id} className="flex items-baseline gap-2.5 text-[14.5px] text-[var(--ink-soft)]">
                  <span className="tabular shrink-0 rounded-sm bg-[var(--card-tint)] px-1.5 py-0.5 text-[11.5px] font-semibold">
                    {AGENDA_KIND[a.kind]}
                  </span>
        ...
        <form action={run} className="shrink-0">
          <button type="submit" className={outlineBtn}>판정</button>
        </form>
      </div>
    </li>
  );
